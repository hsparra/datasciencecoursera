...
#     x <- cleanTextForMatch(str)
    ...
    endCheck <- length(keys) - 1
    if (endCheck < lastCheck) {
        endCheck <- lastCheck
    }
    matches <- tbl[V2 == keys[length(keys)]] [V1 == keys[lastCheck:endCheck]]
...
q4Match <- function(inText, matchTable, wrds) {
    x <- cleanText(inText)
