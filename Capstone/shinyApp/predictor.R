...
selectReturn <- function(m, wrds, numberToReturn = 1) {
    m <- m[order(-ratio)]
}
...
qMatch <- function(inText, m4gram, m3gram, m2gram, wrds, numberToReturn = 1) {
    x <- cleanTextForMatch(inText)
    x <- match(x, wrds)
    if (length(x) == 0) {
        return("")
    ...
    if (length(x) >= 3) {
        ...
        if (dim(m)[1] > 0) {
            m <- m[order(-ratio)]
            return( wrds[m$V4[1:numberToReturn]])
        }
    }
    ...
    if (length(x) >= 2) {
        ...
        if (dim(m)[1] > 0) {
            m <- m[order(-ratio)]
            return( wrds[m$V3[1:numberToReturn]])
        }
    }
