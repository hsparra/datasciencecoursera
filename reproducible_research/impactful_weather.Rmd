...
We will look at the five top event types for causing the most total economic damage over time. First we get the top ten
```{r}
...
```
As can be seen, the top three type of events in terms of total damage have caused many times more damage than any of the other types of events.

Next we look at the five types of severe weather events that have had the largest impact on health.
We will look at the five top event types for causing the most total economic damage over time. First we get the top ten
```{r}
...
```
Here we see one type of event, tornadoes, has caused more injuries and deaths than any other type of event.
...
As can be seen tornadoes have had a disportionate impact on health compared to other severe weather types. Percentage-wise the impact from tornadoes is dominant across the board.
```{r}
library(scales)
...
Tornadoes have caused two thirds of all injuries and almost forty percent of all fatalities. We will look at the top five severe weather types to see if there is another severe weather event that has caused close to as many fatalities as tornadoes.
```{r}
